            ...
            ELSE COALESCE((
              SELECT
                ...
                + COALESCE((SELECT SUM(CASE WHEN UPPER(vd.dr_cr)='CR' THEN vd.amount::numeric ELSE -vd.amount::numeric END)
                            ...
                            WHERE vd.sub_ledger_id = s.sub_ledger_id
                            AND vm.source_type IS DISTINCT FROM 'grn'), 0)
            ...
            ELSE COALESCE((
              SELECT
                ...
                + COALESCE((SELECT SUM(CASE WHEN UPPER(vd.dr_cr)='DR' THEN vd.amount::numeric ELSE -vd.amount::numeric END)
                            FROM voucher_det vd
                            JOIN voucher_mas vm ON vm.id = vd.voucher_mas_id
                            WHERE vd.sub_ledger_id = c.sub_ledger_id
                            AND vm.source_type IS DISTINCT FROM 'job_work_invoice'), 0)
    ...
    try {
      ...
      const slBills = await pool.query(`
        SELECT
          ...
          TO_CHAR(slb.ref_date, 'YYYY-MM-DD') AS bill_date,
...
- "drgNo" is the drawing number for the line item (may be labelled DRG No, Drawing No, DWG No). Empty string if not present.`;
  ...
  // GET /api/reports/job-work-pending — job work inward items not fully despatched
  app.get("/api/reports/job-work-pending", requireAuth, async (req, res) => {
    try {
      const { pool } = await import("./db");
      ...
      const rows = (await pool.query(`
        SELECT
          j.id                AS order_id,
          j.voucher_no        AS order_no,
          j.work_order_no,
          j.inward_date       AS order_date,
          j.party_dc_no,
          j.delivery_date,
          j.despatch_status,
          ...
          i.qty               AS order_qty,
          i.process,
          COALESCE(SUM(di.qty_despatched), 0)        AS despatched_qty,
          i.qty - COALESCE(SUM(di.qty_despatched),0) AS pending_qty
        ...
        WHERE j.despatch_status IN ('Pending', 'Partial')
          AND j.inward_date BETWEEN $1 AND $2
        GROUP BY j.id, j.voucher_no, j.work_order_no, j.inward_date,
                 j.party_dc_no, j.delivery_date, j.despatch_status,
                 c.name, j.party_name_manual,
                 i.id, i.seq_no, i.item_code, i.item_name, i.unit, i.qty, i.process
        HAVING i.qty - COALESCE(SUM(di.qty_despatched),0) > 0
        ORDER BY j.inward_date DESC, j.created_at DESC, i.seq_no
      `, [from, to])).rows;
      res.json(rows);
    } catch (e: any) { res.status(500).json({ message: e.message }); }
  });

  // GET /api/reports/despatch-pending — inward items with outstanding despatch balance
  app.get("/api/reports/despatch-pending", requireAuth, async (req, res) => {
    try {
      const { pool } = await import("./db");
      const from = (req.query.from as string) || "2000-01-01";
      const to   = (req.query.to   as string) || "2099-12-31";
      const rows = (await pool.query(`
        SELECT
          ...
          i.unit,
          i.qty               AS ord_qty,
          COALESCE(SUM(di.qty_despatched), 0)        AS des_qty,
          i.qty - COALESCE(SUM(di.qty_despatched),0) AS pending_qty
        FROM job_work_inward j
        LEFT JOIN customers              c  ON c.id  = j.party_id
        LEFT JOIN job_work_inward_items  i  ON i.inward_id = j.id
        LEFT JOIN job_work_despatch_items di ON di.inward_item_id = i.id
        WHERE j.inward_date BETWEEN $1 AND $2
        GROUP BY j.id, j.voucher_no, j.work_order_no, j.inward_date,
                 j.delivery_date, j.despatch_status,
                 c.name, j.party_name_manual,
                 i.id, i.seq_no, i.item_code, i.item_name, i.unit, i.qty
        HAVING i.qty - COALESCE(SUM(di.qty_despatched),0) > 0
        ORDER BY j.inward_date DESC, j.created_at DESC, i.seq_no
      `, [from, to])).rows;
      res.json(rows);
    ...
    try {
      ...
      const rows = (await pool.query(`
        ...
        opening_txns AS (
          ...
          WHERE NOT EXISTS (SELECT 1 FROM job_work_invoices jwi WHERE jwi.voucher_no = slb.ref_no)
        ),
        txns AS (
          SELECT * FROM invoice_txns UNION ALL
          SELECT * FROM voucher_txns UNION ALL
          SELECT * FROM opening_txns
        ),
        aged AS (
          SELECT *, (CURRENT_DATE - txn_date::date) AS age_days FROM txns
        ),
        -- FIFO netting: apply CR receipts against oldest DR invoices first
        -- Running cumulative DR per customer ordered oldest-first (highest age_days first)
        dr_aged AS (
          SELECT *,
            SUM(amount) OVER (
              PARTITION BY customer_id
              ORDER BY age_days DESC, txn_date ASC
              ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
            ) AS running_dr
          FROM aged WHERE dr_cr = 'DR'
        ),
        cr_total AS (
          SELECT customer_id, COALESCE(SUM(amount), 0) AS total_cr
          FROM aged WHERE dr_cr = 'CR'
          GROUP BY customer_id
        ),
        -- Net balance per DR line after absorbing payments (oldest bills reduced first)
        dr_net AS (
          SELECT
            d.customer_id, d.customer_name, d.contact_no, d.contact_person, d.age_days,
            GREATEST(0::numeric,
              d.amount - GREATEST(0::numeric, COALESCE(c.total_cr, 0) - (d.running_dr - d.amount))
            ) AS net_amount
          FROM dr_aged d
          LEFT JOIN cr_total c ON c.customer_id = d.customer_id
        )
        SELECT
          customer_id,
          customer_name,
          contact_no,
          contact_person,
          COALESCE(SUM(CASE WHEN age_days BETWEEN 0    AND $1-1   THEN net_amount END),0) AS bucket1,
          COALESCE(SUM(CASE WHEN age_days BETWEEN $1   AND $1*2-1 THEN net_amount END),0) AS bucket2,
          COALESCE(SUM(CASE WHEN age_days BETWEEN $1*2 AND $1*3-1 THEN net_amount END),0) AS bucket3,
          COALESCE(SUM(CASE WHEN age_days BETWEEN $1*3 AND $1*4-1 THEN net_amount END),0) AS bucket4,
          COALESCE(SUM(CASE WHEN age_days >= $1*4                  THEN net_amount END),0) AS bucket_above,
          0                                                                                  AS others,
  ...
  // GET /api/reports/supplier-payables — aging report with configurable day buckets
  app.get("/api/reports/supplier-payables", requireAuth, async (req, res) => {
    try {
      const { pool } = await import("./db");
      const days = Math.max(1, parseInt((req.query.days as string) || "15", 10));
      const rows = (await pool.query(`
        ...
        txns AS (
          -- Bills from sub_ledger_bills (CR = payable owed to supplier)
          SELECT
            ss.supplier_id,
          ...
          UNION ALL
          -- Voucher lines (payments DR, journals) against the same sub-ledgers
          SELECT
            ss.supplier_id,
            s.name                                           AS supplier_name,
            COALESCE(s.phone, s.telephone, '')               AS contact_no,
        ...
        -- FIFO netting: apply DR payments against oldest CR bills first
        cr_aged AS (
          SELECT *,
            SUM(amount) OVER (
        ...
        dr_total AS (
          SELECT supplier_id, COALESCE(SUM(amount), 0) AS total_dr
          FROM aged WHERE dr_cr = 'DR'
          GROUP BY supplier_id
        ),
        -- Net outstanding balance per CR bill after absorbing payments (oldest bills first)
        cr_net AS (
          SELECT
            c.supplier_id, c.supplier_name, c.contact_no, c.contact_person, c.age_days,
            GREATEST(0::numeric,
              c.amount - GREATEST(0::numeric, COALESCE(d.total_dr, 0) - (c.running_cr - c.amount))
            ) AS net_amount
          FROM cr_aged c
          LEFT JOIN dr_total d ON d.supplier_id = c.supplier_id
        )
        SELECT
          supplier_id,
          supplier_name,
  ...
  app.get("/api/reports/despatch-register", requireAuth, async (req, res) => {
    try {
      ...
      const rows = (await pool.query(`
        SELECT
          d.id                                              AS despatch_id,
          d.voucher_no                                      AS des_no,
          d.despatch_date                                   AS des_date,
          j.voucher_no                                      AS jw_no,
          j.work_order_no,
          ...
          (di.qty_despatched * di.rate
            + di.cgst_amt + di.sgst_amt + di.igst_amt)     AS total_amt
  ...
  // GET /api/job-work-despatch — list (supports ?party_id= filter)
  ...
  // GET despatch items for invoice by despatch ID — pending qty only (CTE, single pass)
  app.get("/api/job-work-despatch/:id/items-for-invoice", requireAuth, async (req, res) => {
    try {
      const { pool } = await import("./db");
      const r = await pool.query(`
        WITH inv_totals AS (
          SELECT inward_item_id, despatch_id,
                 SUM(qty_despatched) AS invoiced_qty
          FROM job_work_invoice_items
          WHERE despatch_id = $1
          GROUP BY inward_item_id, despatch_id
        )
        SELECT di.id, di.despatch_id, di.inward_id, di.inward_item_id, di.item_id,
               ...
               GREATEST(0, di.qty_despatched - COALESCE(inv.invoiced_qty, 0))
        ...
        LEFT JOIN inv_totals inv ON inv.inward_item_id = di.inward_item_id
                                 AND inv.despatch_id = di.despatch_id
        WHERE di.despatch_id = $1
          AND di.qty_despatched > COALESCE(inv.invoiced_qty, 0)
        ORDER BY di.seq_no
      `, [req.params.id]);
      res.json(r.rows);
    } catch (e: any) { res.status(500).json({ message: e.message }); }
  ...
  // GET inward despatch items for invoice (by inward id) — pending qty only (CTE)
  app.get("/api/job-work-inward/:id/despatch-items-for-invoice", requireAuth, async (req, res) => {
    try {
      const { pool } = await import("./db");
      const r = await pool.query(`
        WITH inv_totals AS (
          SELECT ii.inward_item_id, ii.despatch_id,
                 SUM(ii.qty_despatched) AS invoiced_qty
          FROM job_work_invoice_items ii
          JOIN job_work_despatch_items di ON di.inward_item_id = ii.inward_item_id
                                         AND di.despatch_id = ii.despatch_id
          WHERE di.inward_id = $1
          GROUP BY ii.inward_item_id, ii.despatch_id
        )
        SELECT di.id, di.despatch_id, di.inward_id, di.inward_item_id, di.item_id,
               di.item_code, di.item_name, di.unit, di.process,
                COALESCE(NULLIF(jwii.drg_no,''), NULLIF(prod.drg_no,''), '') AS drawing_no,
  ...
  // GET inward items directly for invoice — pending qty only (CTE, single pass)
  app.get("/api/job-work-inward/:id/direct-items-for-invoice", requireAuth, async (req, res) => {
    try {
      const { pool } = await import("./db");
      const r = await pool.query(`
        WITH inv_totals AS (
          SELECT inward_item_id, SUM(qty_despatched) AS invoiced_qty
          FROM job_work_invoice_items
          WHERE despatch_id IS NULL AND inward_item_id IS NOT NULL
        ...
        LEFT JOIN processes p ON p.id = i.process_id
        LEFT JOIN products prod ON prod.id = i.item_id
        LEFT JOIN inv_totals inv ON inv.inward_item_id = i.id
        WHERE i.inward_id = $1
          AND i.qty > COALESCE(inv.invoiced_qty, 0)
        ORDER BY i.seq_no
      `, [req.params.id]);
      res.json(r.rows);
    } catch (e: any) { res.status(500).json({ message: e.message }); }
