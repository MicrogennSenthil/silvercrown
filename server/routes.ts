    ...
    try {
      ...
      const r = await pool.query(`
        SELECT di.id, di.despatch_id, di.inward_id, di.inward_item_id, di.item_id,
               ...
               di.qty_despatched,
               COALESCE(NULLIF(prod.rate,0), prod.selling_price, 0)               AS rate,
               di.qty_despatched * COALESCE(NULLIF(prod.rate,0), prod.selling_price, 0) AS amount,
    ...
    try {
      ...
      const r = await pool.query(`
        SELECT i.id as inward_item_id, i.inward_id, i.item_id, i.item_code, i.item_name,
               ...
               COALESCE(NULLIF(i.rate,0), NULLIF(prod.rate,0), prod.selling_price, 0)    as rate,
               iw.party_dc_no,
