...
export default function StockReportValue() {
  ...
  const { data: rawRows = [], isLoading } = useQuery<ReportRow[]>({
    queryKey: ["/api/reports/stock-report-value", fromDate, toDate, categoryId, subCategoryId, zeroQtyMode],
    queryFn: () => {
      ...
      return fetch(`/api/reports/stock-report-value?${params}`, { credentials: "include" }).then(r => r.json());
    },
  });
