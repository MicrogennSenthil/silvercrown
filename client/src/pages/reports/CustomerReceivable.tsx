...
export default function CustomerReceivable() {
  const [search,  setSearch]  = useState("");
  const [days,    setDays]    = useState("15");
  const [daysInput, setDaysInput] = useState("15");
  const inputRef = useRef<HTMLInputElement>(null);

  // Parse applied days value
  const appliedDays = Math.max(1, parseInt(days, 10) || 15);

  const { data: apiData, isLoading } = useQuery<{ days: number; rows: Row[] }>({
    queryKey: ["/api/reports/customer-receivable", days],
    queryFn: () =>
      fetch(`/api/reports/customer-receivable?days=${appliedDays}`, { credentials: "include" })
        .then(r => r.json()),
  });
  ...
  // Dynamic column headers based on appliedDays
  const d = appliedDays;
  const colHeaders = [
    `0–${d} Days`,
    `${d}–${d*2} Days`,
  ...
  function handleExcel() {
    const headers = ["S.No", "Customer Name", ...colHeaders, "Contact No", "Contact Person"];
    ...
    exportToCSV(`CustomerReceivable_${d}days.csv`, headers, rows);
  }
  ...
  // Custom "Days" input in the extra filters slot
  const daysFilter = (
              ...
              <tr><td colSpan={11} className="px-5 py-14 text-center">
                <div className="flex flex-col items-center gap-2 text-gray-400">
                  ...
                  <span className="text-sm">
                    {search ? "No customers match the search." : "No outstanding receivables found."}
                  </span>
                </div>
              </td></tr>
            )}
