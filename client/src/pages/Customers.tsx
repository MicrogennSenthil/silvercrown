...
function Field({ label, value, onChange, type = "text", className = "", error = false }: any) {
  if (type === "date") {
...
function normalizeCustomer(s: any) {
  if (!s) return {};
  return {
    name:              s.name              || "",
    ...
    creditLimit:       s.credit_limit      || s.creditLimit      || "",
    creditDays:        s.credit_days       || s.creditDays       || "",
    termOfPayment:     s.term_of_payment   || s.termOfPayment    || "",
                ...
                <tr key={c.id} className="hover:bg-gray-50" data-testid={`row-customer-${c.id}`}>
                  ...
                  <td className="px-4 py-2.5 text-right whitespace-nowrap">
                    {(() => {
                      ...
                      const type = c.outstanding_balance_type || "Dr";
                      if (!c.sub_ledger_id || bal === 0) return <span className="text-gray-300 text-xs">—</span>;
                      const color = bal > 0 ? SC.primary : "#d74700";
                      return (
