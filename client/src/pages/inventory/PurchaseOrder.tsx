...
type PoItem = {
  ...
  tax_code: string;
  cgst_pct: string; sgst_pct: string; igst_pct: string;
  cgst_amt: string; sgst_amt: string; igst_amt: string;
  total: string;
};
type PoTerm   = { _key: string; term_type: string; terms: string };
type PoCharge = { _key: string; charge_type: string; amount: string };
...
function PoForm({ editData, onBack }: { editData?: any; onBack: () => void }) {
  ...
  const [terms, setTerms]     = useState<PoTerm[]>(
    editData?.terms?.length ? editData.terms.map((t: any) => ({ _key: crypto.randomUUID(), term_type: t.term_type||"", terms: t.terms||"" })) : [newTerm()]
  );
  const [charges, setCharges] = useState<PoCharge[]>(
    editData?.charges?.length ? editData.charges.map((c: any) => ({ _key: crypto.randomUUID(), charge_type: c.charge_type||"", amount: String(c.amount||"0") })) : [newCharge()]
  );
  ...
  useEffect(() => {
    if (!isEdit) {
      fetch("/api/voucher-series/next/purchase_order", { credentials: "include" })
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      const url = isEdit ? `/api/purchase-orders/${editData.id}` : "/api/purchase-orders";
    ...
    <div ref={containerRef} className="p-4 md:p-6" style={{ background: SC.bg, minHeight: "100vh", fontFamily: "Source Sans Pro, sans-serif" }}>
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-sm">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {/* Row 1 — PO No | PO Date | Supplier Name | PO Type */}
          <div className="grid grid-cols-4 gap-3">
            <div className="relative">
              <label className="absolute -top-2 left-3 bg-white px-1 text-xs text-gray-500 z-10 leading-none">PO No</label>
              <input value={voucherNo} readOnly
                ...
                style={{ color: SC.primary }} data-testid="input-po-no"/>
            </div>
                ...
                <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-30 max-h-48 overflow-y-auto mt-0.5">
                  {filteredSuppliers.slice(0, 8).map((s: any) => (
                    <button key={s.id} onClick={() => { setSuppId(""); setSuppSearch(s.name); setSuppOpen(false); }}
                      className="w-full text-left px-3 py-2 text-sm hover:bg-[#d2f1fa]" data-testid={`opt-sup-${s.id}`}>
                      {s.name}
                    </button>
                  ))}
                </div>
            ...
            <>
              <div className="border border-gray-200 rounded-lg overflow-visible">
                ...
                <div className="grid text-xs font-semibold text-gray-600 bg-gray-50 border-b"
                  style={{ gridTemplateColumns: "36px 90px 1fr 60px 55px 70px 90px 80px 90px 90px 80px 90px 36px" }}>
                  {["S.No","Item Code","Item Name","Qty","Unit","Rate ₹","Taxable ₹","Tax Code","CGST ₹","SGST ₹","IGST ₹","Total ₹",""].map(h => (
                ...
                {items.map((row, idx) => {
                  ...
                  return (
                    <div key={row._key} className="grid items-center border-b last:border-0 hover:bg-gray-50"
                      ...
                      {/* Item Code — read-only, filled by picking item */}
                      ...
                      {/* Tax Code */}
                      <div className="px-1 py-1">
                        <select value={row.tax_code}
                          onChange={e => updateItemField(row._key,"tax_code",e.target.value)}
                          className="w-full border border-gray-200 rounded px-1 py-1 text-xs outline-none focus:border-[#027fa5] bg-white"
                          data-testid={`sel-tax-${idx}`}>
                          <option value="">None</option>
                          {Object.keys(TAX_CODES).map(k => <option key={k} value={k}>{k}</option>)}
                        </select>
                      </div>
                      ...
                      <div className="px-1 flex justify-center">
                        {idx === items.length-1
                          ? <button onClick={() => setItems(p => [...p, newItem()])}
                              className="text-green-600 hover:text-green-800 p-1" data-testid={`btn-add-${idx}`}><Plus size={13}/></button>
                          : <button onClick={() => setItems(p => p.filter(r => r._key!==row._key))}
                              className="text-red-400 hover:text-red-600 p-1" data-testid={`btn-rm-${idx}`}><Trash2 size={13}/></button>
                        }
                      </div>
                    </div>
                  );
              ...
              <div className="flex items-center justify-between px-2 py-1 text-sm border-t border-gray-100">
                <div className="flex items-center gap-4">
                  <button onClick={() => setItems([newItem()])}
                    className="text-xs text-red-500 hover:text-red-700 font-semibold" data-testid="btn-remove-all">
                    Remove all
                  </button>
                  ...
                  <span className="text-gray-600 text-xs">
                    Tax Amount : <span className="font-bold text-gray-800">{totalTax > 0 ? `₹ ${n2(totalTax)}` : "—"}</span>
                  </span>
                </div>
                <span className="font-bold text-gray-800 text-sm">
                  Grand Total : <span style={{ color: SC.primary }}>₹ {n2(grandTotal)}</span>
                </span>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="flex items-center justify-between px-2 py-1 text-sm border-t border-gray-100">
                <span className="text-xs text-gray-600">
                  Total Amount : <span className="font-bold text-gray-800">{totalTaxable > 0 ? `₹ ${n2(totalTaxable)}` : "—"}</span>
                </span>
                <span className="text-xs text-gray-600">
                  Tax Amount : <span className="font-bold text-gray-800">{totalTax > 0 ? `₹ ${n2(totalTax)}` : "—"}</span>
                </span>
...
// ── List View ────────────────────────────────────────────────────────────────
function PoList({ onNew, onEdit }: { onNew: () => void; onEdit: (d: any) => void }) {
  ...
  const filtered = (list as any[]).filter((r: any) => {
    ...
    return (r.voucher_no||"").toLowerCase().includes(s) || (r.supplier_name_db||r.supplier_name_manual||"").toLowerCase().includes(s);
  });

  const statusBadge = (status: string) => {
    const styles: Record<string,string> = { Draft:"bg-yellow-100 text-yellow-800", Approved:"bg-green-100 text-green-700", Cancelled:"bg-red-100 text-red-700" };
    return <span className={`px-2 py-0.5 rounded text-xs font-semibold ${styles[status]||"bg-gray-100 text-gray-600"}`}>{status||"Draft"}</span>;
  };
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-sm">
        ...
        <div className="px-6 py-3 border-b border-gray-100">
          <div className="relative max-w-xs">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search PO no or supplier..."
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                {["PO No","PO Date","Supplier Name","PO Type","Schedule Date","Priority","Payment","Status","Actions"].map(h => (
            ...
            <tbody>
              ...
              {!isLoading && filtered.length===0 && <tr><td colSpan={9} className="px-4 py-8 text-center text-sm text-gray-400">No purchase orders found</td></tr>}
              {filtered.map((r: any) => (
                <tr key={r.id} className="border-b last:border-0 hover:bg-[#f0f9ff] transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-700">{fmt(r.po_date)}</td>
                  <td className="px-4 py-3 text-gray-700">{r.supplier_name_db||r.supplier_name_manual||"—"}</td>
                  <td className="px-4 py-3 text-gray-600">{r.po_type}</td>
...
export default function PurchaseOrder() {
  ...
  async function handleEdit(row: any) {
    const res = await fetch(`/api/purchase-orders/${row.id}`, { credentials: "include" });
    setEditData(await res.json());
    setView("form");
  }

  if (view==="form") return <PoForm editData={editData} onBack={() => { setEditData(null); setView("list"); }}/>;
  return <PoList onNew={() => { setEditData(null); setView("form"); }} onEdit={handleEdit}/>;
}
