...
function PoForm({ editData, onBack }: { editData?: any; onBack: () => void }) {
  ...
  const products = rawMatCatIds.size > 0 ? (allProductsRaw as any[]).filter((p: any) => rawMatCatIds.has(p.categoryId)) : allProductsRaw as any[];
    ...
    <div ref={containerRef} className="p-4 md:p-6" style={{ background: SC.bg, minHeight: "100vh", fontFamily: "Source Sans Pro, sans-serif" }}>
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-sm">
            ...
            <>
              <div className="border border-gray-200 rounded-lg overflow-visible">
                ...
                {items.map((row, idx) => {
                  ...
                  const filtered = search ? (products as any[]).filter((p: any) =>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100">
          ...
          <button onClick={() => {
            if (!suppSearch.trim()) { toast({ title:"Supplier required", description:"Please select or type a supplier.", variant:"destructive" }); return; }
            saveMut.mutate();
